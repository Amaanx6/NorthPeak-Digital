...
export default function Navbar() {
  ...
  useEffect(() => {
    if (hovered && linkRefs.current[hovered] && containerRef.current) {
    ...
    <>
      <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-5">
        ...
        >
          ...
          >
            <motion.div
              className="absolute top-1/2 h-8 -translate-y-1/2 rounded-pill bg-ridge"
              animate={{
                x: pill.x,
                width: pill.width,
                opacity: hovered ? 1 : 0,
            ...
            {NAV_LINKS.map((link) => (
              <a
                ...
                className="relative z-10 rounded-pill px-4 py-2 text-sm font-medium text-ink-soft transition-colors hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-alpine-400"
              >
                {link.label}
              </a>
            ))}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-3 py-2">
                <span className="font-display text-[15px] font-semibold text-ink">{SITE_NAME}</span>
                <button
                  onClick={() => setMobileOpen(false)}
      ...
      <div className="h-[92px] w-full" aria-hidden="true" />
    </>
  );
}
